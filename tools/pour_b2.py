#!/usr/bin/env python3
"""
pour_b2.py -- the CH2 leg-B coil path, bridge midpoint to the yoke connector.

    py tools\\pour_b2.py            dry run
    py tools\\pour_b2.py --write    back up, patch, verify, write

NO VIA FIELD HERE. Every pad carrying the 30 A on this net is through-hole --
Q203 pin 3, Q204 pin 2, J204 pin 2 -- so a B.Cu pour reaches all three down their
...
STILL UNCONNECTED AFTER THIS RUNS: U207.6, C226.2 and R224.1 -- the high-side
...
0.64 mm away:
    ...
    Q203.2 +60V 191.59-193.49        Q204.1 gate 204.05-205.95
    Q203.3 COIL 194.13-196.03        Q204.2 COIL 206.59-208.49
                                     Q204.3 GND  209.13-211.03
...
pattern pour_a2 settled on after KiCad rejected equal priorities.
    ...
    Ba  x[193.8,208.5]  y[63.4,138.2]   vertical run, 14.7 mm wide
    Bb  x[193.0,246.2]   y[127.5,138.2]  east along the clear band, 10.7 mm
    Bc  x[235.5,246.2]   y[137.5,161.5]  drop onto J204.2, 10.7 mm
    L1  x[193.83,196.33] y[60.5,64.0]    lobe to Q203.3
    L2  x[206.29,208.79] y[60.5,64.0]    lobe to Q204.2

The vertical is 14.7 mm rather than B1's 15.0 because TP206 sits 0.85 mm off its
right edge at (210.35, 112.4). The east band stops at x 246.2, which leaves
1.30 mm to the COIL_A2 pour running down the board edge -- the two coil nets of
channel 2 pass within 1.3 mm of each other for 10.7 mm, both at bus potential
and both switching, so that gap is worth remembering when the board is probed.

The y 127.5-138.2 band is free of through-hole pads across its whole 44 mm
span: the CH2 test points sit above it at y 112-119, the bulk cap row below at 143.
...
ZONES = {'COIL_B2_Ba': (193.8, 208.5, 63.4, 138.2, 16),
         'COIL_B2_Bb': (193.0, 246.2, 127.5, 138.2, 17),
         'COIL_B2_Bc': (235.5, 246.2, 137.5, 161.5, 18),
         'COIL_B2_L1': (193.83, 196.33, 60.5, 64.0, 19),
         'COIL_B2_L2': (206.29, 208.79, 60.5, 64.0, 20)}
LAYER = 'B.Cu'
BOND_PADS = [('Q203', '3'), ('Q204', '2'), ('J204', '2')]
