#!/usr/bin/env python3
"""
place_review.py -- the eight moves from the block-by-block placement review.

    py tools\\place_review.py            dry run: computes and prints, writes nothing
    py tools\\place_review.py --write    back up, patch, verify, write

WHY
Walking the board block by block (docs/20 section 20.9) turned up eight parts
sitting away from the pin they exist to serve:

    C211   differential filter cap 7.05 mm from the INA240 inputs, because
           U203 was rotated 180 for the CH2 mirror and the cap did not follow
    C25 C26 C235 C237   comparator threshold caps, 5.6 to 12.8 mm from the
           comparator inputs whose reference they are supposed to hold steady
    C27 C239   the RC that filters the current signal into the comparator,
           10.5 mm from the pin it filters
    R238   PWM pulldown 46.9 mm from its driver while its three siblings are
           at 25 to 28 mm, again because CH2's legs are swapped and this part
           was translated rather than mirrored

Four of the eight are the same underlying mistake: a block mirrored for CH2
where one part kept CH1's geometry.

Positions were found by searching outward from each target pin on a 0.05 mm
grid, rejecting anything whose courtyard collides, leaves the board outline by
less than 0.5 mm, or intrudes on the heatsink shadow.

SAFETY
Refuses to run while KiCad holds the lock file. Reads and writes bytes so CRLF
survives. Re-parses its own patched buffer, recomputes every courtyard and
every target distance from the patched geometry, and writes nothing unless
they agree.
"""
...
MOVES = {
    'C211': (250.50, 84.50,   0, ('U203', '8'),  4.0),
    'C25':  (115.75, 112.65, 90, ('U4',   '3'),  3.0),
    'C26':  (125.20, 112.65, 90, ('U4',   '6'),  3.0),
    'C27':  (115.70, 108.85, 90, ('U4',   '2'),  3.5),
    'C235': (213.30, 110.70, 270,('U204', '3'),  3.0),
    'C237': (222.70, 110.70, 270,('U204', '6'),  3.0),
    'C239': (214.60, 106.75, 180,('U204', '2'),  3.5),
    'R238': (182.00, 97.50,   0, ('U207', '1'), 28.0),
}
# per-move limit is the fifth field of MOVES
...
print('parts requested: %d' % len(MOVES))
print('parts patched       : %d  ->  %s' % (len(touched), ' '.join(sorted(touched))))
...
ok = True
for ref, (x, y, ang, (tref, tpad), limit) in sorted(MOVES.items()):
    tp = before[tref]['pads'][tpad][0]
    sig = [pn for pn, (p, nt) in after[ref]['pads'].items() if nt != 'GND']
    was = min(math.hypot(before[ref]['pads'][pn][0][0] - tp[0],
                         before[ref]['pads'][pn][0][1] - tp[1])
              for pn in before[ref]['pads'])
    now = min(math.hypot(after[ref]['pads'][pn][0][0] - tp[0],
                         after[ref]['pads'][pn][0][1] - tp[1]) for pn in sig)
    flag = '' if now <= limit else '   OVER LIMIT'
    print('  %-5s -> %-8s  %7.2f mm  ->  %5.2f mm  (limit %.1f)%s'
          % (ref, tref + '.' + tpad, was, now, limit, flag))
    if now > limit:
        ok = False
