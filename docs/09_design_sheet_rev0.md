...
| A10 | Board outline | **160 × 140 mm** — grew from 160 × 100 (Eurocard) on 2026-08-21 | Decided 2026-08-14 as 160 × 100, no upstream constraint stated; set by the 8× TO-220 row (~120 mm) and the §4.2 heatsink estimate (~150 × 100 mm). Grew to 140 mm tall because the six Ø16 × 31.5 mm bulk cans did not fit between the coil connectors — `20_layout_phase.md` §20.1 | Decision, not measurement | Outline changes | Re-do placement. No schematic impact. Cheapest thing here to change *before* routing, expensive after |
| A11 | Mounting | **4 × M3**, Ø3.2 mm, on a **150 × 130 mm** rectangle: H1 (105,55) H2 (255,55) H3 (105,185) H4 (255,185) | Decided 2026-08-14 as 150 × 90 when the board was 160 × 100; the holes moved with the board's growth to 160 × 140 and this entry was corrected 2026-08-24 to match the file. Nearest part 9.60 mm, edge margin 5 mm | Measured from the board, not a decision | Hole pattern moves | Move 4 holes + keepouts. Trivial before routing |
| A12 | Chassis bond | Mounting holes are `MountingHole:MountingHole_3.2mm_M3_Pad`: a **plated 3.2 mm hole with a 6.4 mm annular pad on all four copper layers**, currently carrying **no net**. Verified 2026-08-27 by reading the pad block. The 0 Ω 1206 jumper this row used to claim was specified and never drawn, and no 0 Ω part exists in the netlist, but the hole itself is a better bonding provision than that jumper would have been. The `no net found ... no pin 1 in symbol` warning on update-from-schematic is the symbol's doing: `MountingHole` has no pin, so the pad cannot inherit a net | Yokes float; the only common point is at the motor-controller negatives (`03_open_questions.md`). A bonded screw would create an unplanned 60 A return path through the pod structure, so floating is still the intended state | Medium | If chassis bonding is required | **Swap the symbol to `MountingHole_Pad` and assign the pad to GND.** Schematic edit and a re-push, no board re-spin: the copper is already there on all four layers. For a bench trial before committing, solder a wire from a GND test point to the screw (TP212 to H4 is the shortest run at 56 mm). Note for layout: because the pad has no net, every zone treats it as a foreign obstacle, which is what forces the COIL_A1 pour to keep its distance from H1 |
...
| A15 | Stackup dielectrics | F.Cu→In1.Cu prepreg **0.2 mm**; core **0.97 mm**; In2.Cu→B.Cu prepreg 0.2 mm. Copper 2 oz outer / 1 oz inner, board total **1.600 mm** | Decided 2026-08-24, entered in Board Setup. Standard 1.6 mm 4-layer construction, **not** a fab’s published table — 0.97 mm is not a stocked core (0.8/0.9/1.0/1.1/1.2 are), it is the residue that makes the total land on 1.600. Reasoning in `21_stackup_and_layers.md` | Decision, not a quote | Only `dielectric 1` matters: loop inductance over the plane goes as μ₀·h/w, so a larger F.Cu→In1.Cu separation scales overshoot linearly (≈1.5 V per 10 mm of run per 1.24 mm of extra separation, at 30 A / 30 ns). Separately, JLCPCB’s **default inner copper on 4 layers is 0.5 oz**, not 1 oz — if 1 oz inner is not explicitly ordered, the In2.Cu parallel-copper saving falls from 9.30→6.20 W to 9.30→7.44 W | Replace all three dielectric rows with the fab’s actual stackup table at quote time. Board Setup edit only, no layout impact. Order 1 oz inner explicitly |
| A16 | HV net class sizing | `HV_POWER` track width **10.7 mm**, clearance **0.3 mm**, via 1.2 / 0.6. `GND` split out to `PWR_GND` (via 1.2 / 0.6); bootstrap split out to `HV_BOOT` (0.5 mm, via 0.8 / 0.4) | Decided 2026-08-24. 10.7 mm is the IPC-2221 external width for 30 A at 2 oz, ΔT = 20 °C — the class now states the sized width instead of 3.0 mm, which is ~160 °C at 30 A. 0.3 mm clearance is against IPC-2221B Table 6-1 at 51–100 V: coated external needs 0.13 mm, so 0.3 mm is 2.3×. The uncoated column (0.6 mm) applies only at pad openings, and the tightest 60 V pad pair on the board measures **0.635 mm** — the TO-220 lead pitch at Q1/Q2/Q202/Q203, 89 HV pads examined | Standard + measurement, not a guess | Conformal coating, operation above 3050 m, or a bus above 100 V all move the IPC column, and the 0.635 mm TO-220 gap becomes the binding number at 6 % margin | Net class values are a Board Setup edit. The 0.635 mm gap is set by the FET package and would need a different footprint |
| A17 | Shared bus section current | The bulk-to-bridge feed carries **both** channels, up to **60 A**. **Corrected 2026-08-27: the 28.0 mm figure in the previous version of this row was wrong**, because it applied the IPC-2221 *trace* formula to what is a *plane*. On In2.Cu at 1 oz that same formula demands **146 mm** of width, which is the whole board -- a result that says the formula is outside its validity, not that the board is impossible. A plane is counted in squares: 1 oz copper is 0.494 mOhm per square, the cap row at y=143 to the bridge row at y=62 is 81 mm across roughly 150 mm of width, so **0.54 squares = 0.267 mOhm**. Adding spreading resistance at the pads (0.026 mOhm across the six bulk caps in parallel, 0.165 mOhm at one FET drain pin) gives about **0.46 mOhm end to end: 27 mV and 1.65 W at 60 A**, spread over 150 x 80 mm, which is 0.14 mW/mm2 | Derived 2026-08-24, corrected 2026-08-27. Every width in the project until then was sized for 30 A as a trace, and the section before the bus splits was first sized the same way | Arithmetic, and the first version of it used the wrong model | The corridor width is **not** the constraint it was thought to be. In2.Cu carries the bus as a plane; every +60 V pad that carries current is through-hole (4 FET drains, 6 bulk caps, J101), so the plane reaches all of them down their own barrels with no vias in the power path. **In2.Cu still cannot be used for signal routing**, for the original reason: cutting it up would turn a plane back into traces | Re-derive if the stackup changes to 0.5 oz inner, where the sheet resistance doubles to 0.99 mOhm/square and the numbers above double with it |
