...
| A17 | Shared bus section current | The bulk-bank-to-bridge feed carries **both** channels, up to **60 A**, needing **28.0 mm** of 2 oz outer at ΔT = 20 °C against 10.7 mm for one channel | Derived 2026-08-24. Every width in the project until now was sized for 30 A, which is one coil. The section before the bus splits was never sized | Arithmetic, not a decision | The clear corridor up the middle of the board is 13 mm, so F.Cu alone cannot carry it. It is met by In2.Cu as a near solid +60 V plane paralleled with F.Cu pours, which means **In2.Cu cannot be used for signal routing** | Re-derive against IPC-2152, whose internal-layer data is less conservative than the IPC-2221 curve used here. Board Setup and zone edits only |
